...
# In this script file, we show how the airline revenue management problems discussed in this lecture can be solved in R
# "lpSolveAPI" package allows you to solve linear optimization problems in R using the lp_solve library, which is an open-sourced optimization solver. 
# If you are solving larger and more complex optimization problems, lp_solve might not be powerful enough to solve your problems.
...
# Two decision variables: the number of regular seats to offer, and the number of discount seats to offer. 
# The first step is to create a model, which takes as arguments the number of constraints in your model, and the number of decision variables in your model.
...
# Now we need to set up the constraints and objective. The best way to do this using the lpSolveAPI package is by viewing the constraints in a matrix format.
# max         617*R + 238*D
# subject to    1*R +   1*D <= 166
#               1*R	+   0*D <= 100
#               0*R +   1*D <= 150	

# So the first column in our constraint matrix is c(1,1,0), and the second column is c(1,0,1) and set the right-hand-side values to c(166,100,150):
...
# You should see that there is a row called "Lower" in the model output - the values of zero indicate that our decision variables are constrained to be non-negative,
# which is the default setting. If you want to change the lower or upper bounds for a decision variable, you can do so with the set.bounds function.
