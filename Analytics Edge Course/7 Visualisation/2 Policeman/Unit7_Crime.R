...
# Save this table as a data frame, so that we can pass it to ggplot later
...
# Adding the Hour of the Day, making a heatmap
...
# Create out plot:
ggplot(DayHourCounts, aes(x=Hour, y=Freq)) + geom_line(aes(group=Var1)) # 7 lines each for a particular day

# Change the colors
ggplot(DayHourCounts, aes(x=Hour, y=Freq)) + geom_line(aes(group=Var1, color=Var1), size=1) # making clear separation
...
# Removing points with 0 frequency or 0 crime incidents
...
# Redo the plot, removing any states with murder rates above 10 (as only washington DC is above 10)
